...
proc sigmoid*(self: ref Model; x: float32): float32 {.noSideEffect.} =
  if x < -(float32)MAX_SIGMOID:
    ...
    var i: int64 = int64( (x + MAX_SIGMOID.float64) *
        SIGMOID_TABLE_SIZE.float64 / MAX_SIGMOID.float64 / 2)
    return self.t_sigmoid.idata[i]

proc binaryLogistic*(self: ref Model; target: int32; label: bool;
    lr: float64): float32 =
...
proc hierarchicalSoftmax*(self: ref Model; target: int32;
    lr: float64): float32 =
...
proc computeOutputSoftmax*(self: ref Model; hidden: var Vector;
    output: var Vector) {.noSideEffect.} =
...
proc computeHidden*(self: ref Model; ipt: var seq[int32];
    hidden: var Vector) {.noSideEffect.} =
...
proc dfs*(self: ref Model; k: int32; threshold: float32; node: int32; score: float32;
  ...
  f = 1.0 / (1 + exp(-f))
  self.dfs(k, threshold, self.tree[node].left, score + self.stdLog(1.0 - f),
      heap, hidden)
...
proc predict*(self: ref Model; ipt: var seq[int32]; k: int32;
    threshold: float32; heap: var seq[tuple[first: float32, second: int32]];
    hidden: ptr Vector; output: ptr Vector) {.noSideEffect.} =
  if k <= 0:
...
proc predict*(self: ref Model; ipt: var seq[int32]; k: int32; threshold: float32;
       heap: var seq[tuple[first: float32, second: int32]]) =
  self.predict(ipt, k, threshold, heap, self.hidden.unSafeAddr,
      self.output.unSafeAddr)
