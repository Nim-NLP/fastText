...
proc assign_centroid*(self: ProductQuantizer; x: ptr float32; c0: ptr float32;
        code: ptr uint8; d: int32): float32 =
    var c = c0
...
proc Estep*(self: ProductQuantizer; x: ptr float32; centroids: ptr float32;
        codes: ptr uint8; d: int32; n: int32) =
    for i in 0..<n:
        discard self.assign_centroid(x[i * d], centroids, codes[i], d)

proc MStep*(self: ProductQuantizer; x0: ptr float32; centroids: ptr float32;
        codes: ptr uint8; d: int32; n: int32) =
    ...
    for k in 0..<ksub:
        if (nelts[k] == 0):
            m = 0
            while (rng1.rand(1.0) * (n - ksub).toFloat >= cast[float](nelts[
                    m] - 1)):
                m = (m + 1) mod ksub
            nimCopyMem(centroids[k.int32 * d], centroids[m*d],
                    sizeof(float32)*d)
            for j in 0'i32..<d:
                sign = (j mod 2) * 2 - 1;
...
proc kmeans(self: var ProductQuantizer; x: ptr float32; c: ptr float32;
    ...
    r.shuffle(perm)
    for i in 0..<ksub:
        nimCopyMem(self.centroids[i.int32 * d].unsafeAddr,
                perm[i*d].unsafeAddr, sizeof(float32)*d)
...
proc compute_code*(self: var ProductQuantizer; x: ptr float32; code: ptr uint8) {.
    noSideEffect.} =
    var d = self.dsub
    var i: int32
    for m in 0..<self.nsubq:
