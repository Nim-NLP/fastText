...
proc pushHash(self: Dictionary; hashes: var seq[int32];
    id: int32) {.noSideEffect.} =
...
proc computeSubwords*(self: Dictionary; word: string; ngrams: var seq[int32];
  ...
  for i in 0..<word.len():
    ...
    while (j < word.len() and n <= self.args.maxn):
      ...
      if (n >= self.args.minn and not (n == 1 and (i == 0 or j ==
          word.len()))):
...
proc `discard`*(self: Dictionary; id: int32;
    rand: float32): bool {.noSideEffect.} =
  assert(id >= 0)
...
proc getSubwords*(self: Dictionary; word: string; ngrams: var seq[int32];
    substrings: var seq[string]) {.noSideEffect.} =
...
proc getLabel*(self: Dictionary; lid: int32): string {.noSideEffect.} =
  if lid < 0 or lid >= self.nlabels:
    raise newException(ValueError, "Label id is out of range [0, " &
        self.nlabels.intToStr)
...
proc getLine*(self: Dictionary; i: Stream; words: var seq[int32];
  ...
  for token in tokens:
    ...
    if wid < 0: continue
    inc ntokens
    if self.getType(wid) == entry_type.word and self.discard(wid, rng.rand(
        1.0)) == false:
      words.add(wid)
